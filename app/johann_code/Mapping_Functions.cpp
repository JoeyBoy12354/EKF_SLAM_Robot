...
namespace Mapping_Functions{
    ...
    void storeMapPoints(vector<CarPoint> lidardata,Matrix<float, dim, 1> State){
        float distance_threshold = 100;//If two points are greater than Xmm then keep this point.
    ...
    void gridMakeDots(vector<CarPoint> mapdata, vector<vector<GridPoint>>& points){
        ...
        float boundThresh = 220;//If distance between gridPoint and lidarPoint <= Xmm then return false 
