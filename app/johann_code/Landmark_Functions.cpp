...
namespace Landmark_Functions{
    ...
    vector<Line> RANSAC2(vector<CarPoint> laserdata){
        ...
        const int RANSAC_CONSENSUS = 80; //RANSAC: at least X points in consensus required to determine if a line will be deemed valid 
    ...
    vector<CarPoint> ANSAC_CORNER(vector<CarPoint> laserdata){
        ...
        const float ANGLE_THRESHOLD_HIGH = 137*PI/180; //If angle made by intercepts is within PI/2 +- X then keep corner
